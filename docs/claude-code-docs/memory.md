...
| **コアファイル・ユーティリティ** | 重要なファイルの場所と役割 |
| **コードスタイル** | `ES modules使用、CommonJS禁止` |
| **テスト方法** | テストの実行方法、アプローチ |
| **リポジトリ規則** | ブランチ命名、merge vs rebase |
| **開発環境セットアップ** | 必要なツール、バージョン |
| **警告・注意点** | プロジェクト固有の罠や癖 |
| **セッション間で覚えておくべき情報** | |
...
| 項目 | 理由 |
|------|------|
| **大量の未検証コンテンツ** | 効果を確認せずに追加しない |
| **冗長・自明な情報** | コンテキストウィンドウを浪費 |
| **プロジェクトに無関係な内容** | |
