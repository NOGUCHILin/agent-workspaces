...
8. **【完了】消費税還付金の徹底調査**（約56万円還付見込み）
9. **【完了】Kintoneの不具合や修理の内容を全て適切なものに変更**

--- 優先度：高（すぐやる） ---
10. **【テスト中】朝の金額KPI入力のスピード化** → `/morning-kpi`
11. **【新規】査定結果送信のスキル化**
12. **【バグ修正・スピード化】査定のスキル化** → `/assessment`（データ整理部分完了、連絡・結果連絡は今後追加）

--- 優先度：中（上記完了後） ---
13. **【新規】ジャンク品の買取価格を全て確認し、変更**
14. **【バグ修正・スピード化】財務管理系スキル化**（残り確認のみ）
15. **【スピード化】夜の金額KPI入力** → `/evening-kpi`
16. **【スピード化】資金繰り表入力** → `/cashflow-update`
17. **【スピード化】タスク設定** → `/task-setup`
18. **【完了】BMコンディションチェックのスピード化** → `/bm-condition-check`
19. **【スピード化】価格調整** → `/price-adjustment`
...
開発・スクリプト作成・スキル実行中に問題（エラー、想定外の挙動、ライブラリの仕様不明など）が発生して進めなくなった場合、以下の手順を**必ず**実行すること。

### 🔴 必須: 問題発生時のフロー

1. **まずWeb検索で情報を収集する**
   - `WebSearch`ツールを使い、エラーメッセージやキーワードで検索する
   - 検索クエリ例: `"エラーメッセージそのまま"`, `"ライブラリ名 使い方"`, `"API名 パラメータ"` など
   - 英語で検索した方が情報が多い場合は英語で検索する

2. **公式ドキュメントを確認する**
   - 検索結果から公式ドキュメントやGitHubのIssueがあれば`WebFetch`で内容を確認する
   - Stack Overflowや技術ブログの解決策も参考にする

3. **収集した情報をもとに解決を試みる**
   - 得られた情報をもとにコードを修正・再実行する
   - 1回の検索で解決しない場合は、キーワードを変えて再検索する

4. **それでも解決しない場合はGrokで検索する**
   - WebSearch・WebFetchで解決できなかった場合の最終手段
   - `WebFetch`で `https://x.com/i/grok?text=質問内容` にアクセスして情報を取得する
   - リアルタイム情報やX上の開発者の知見が得られる可能性がある
   - Grokの回答も参考にしつつ、公式ドキュメントと照合して正確性を確認する

### 🚨 禁止事項

- **推測だけで対処しない** — 不明な点があればまずWeb検索する
- **ユーザーにすぐ「わかりません」と言わない** — 最低1回はWeb検索してから報告する
- **同じエラーで何度もリトライしない** — 2回同じエラーが出たらWeb検索に切り替える

### 検索が必要な場面の例

| 状況 | 検索すべきこと |
|------|--------------|
| Pythonライブラリのエラー | エラーメッセージ + ライブラリ名 |
| APIのレスポンスが想定外 | API名 + エンドポイント + パラメータ |
| Playwrightのセレクタが効かない | サイト名 + 要素の特定方法 |
| npm/pipのインストールエラー | パッケージ名 + エラーメッセージ + OS |
| 型エラー・構文エラー | 言語名 + エラーメッセージ |
| 外部サービスの仕様変更 | サービス名 + API + 最新仕様 |
</problem-solving-rules>
